...
class CocoDataset(BaseDataset):
    ...
    def get_train_data(self, idx):
        ...
        """
        masks = []
        img_segment=cv2.imread(image_path)
        seg_path = os.path.join(self.seg_path, file_name)
        mask=plt.imread(seg_path)*255
        img_segment=cv2.resize(img_segment,(320,320))
        img_segment = np.asarray(img_segment).astype('float')
        img_segment = torch.as_tensor(img_segment) / 255.0
        img_segment = img_segment.permute(2,0,1)
