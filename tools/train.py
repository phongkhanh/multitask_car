...
#python train.py ../config/nanodet_custom_xml_dataset.yml
import os
import torch
import argparse
import numpy as np
...
def parse_args():
    ...
    parser.add_argument('config', default='../config/nanodet_custom_xml_dataset.yml',help='train config file path')
...
def main(args):
    ...
    # for meta in train_dataloader:
    #   img =meta['img_segment']
    #   target = meta['masks']
    #   print(img.shape, target.shape)
    #   #print(target[0].min(), target[0].max())
    #   print(np.unique(target[0]))
    #   mask = np.where(target[0] == 1,255, 0)
